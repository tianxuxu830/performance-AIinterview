...
import { X, Search, CheckSquare, Square, Filter, Calendar, BarChart3, Trophy, Briefcase } from 'lucide-react';
...
const EmployeeSelectorModal: React.FC<EmployeeSelectorModalProps> = ({ isOpen, onClose, onSelect, initialSelectedIds = [], filterTask }) => {
  ...
  // Prepare the display data: Join Records with Employee Info
  const displayData = MOCK_PERFORMANCE_RECORDS.map(record => {
  ...
  // Filter Logic
  const filteredRecords = displayData.filter(item => {
      // 1. Fuzzy Search
      const matchesSearch = 
      ...
      // 2. Period Filter
      const matchesPeriod = selectedPeriod ? item.period === selectedPeriod : true;

      // 3. Task Filter
      const matchesTask = selectedTask ? item.taskName === selectedTask : true;
  ...
  const handleConfirm = () => {
    // Map selected Record IDs back to Employee IDs
    const selectedEmpIds = Array.from(selectedRecordIds).map(rId => {
  ...
  const handleSelectAllFiltered = () => {
     const newSet = new Set(selectedRecordIds);
     filteredRecords.forEach(rec => newSet.add(rec.id));
     setSelectedRecordIds(newSet);
  };
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="px-6 py-2 bg-gray-50/50 border-b border-gray-200 flex justify-between items-center">
             <div className="text-xs text-gray-500">
                 共找到 <span className="font-bold text-gray-800">{filteredRecords.length}</span> 条考核记录
             </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-0">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          <div className="h-9 w-9 rounded-full bg-gray-200 flex-shrink-0 mr-3 overflow-hidden border border-gray-100">
                             <img src={item.employee?.avatar} alt={item.employee?.name} className="h-full w-full object-cover" />
                          </div>
                          <div>
                            <div className="font-medium text-gray-900 text-sm">{item.employee?.name}</div>
                            <div className="text-xs text-gray-500">ID: {item.employee?.id.padStart(4, '0')}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-900">{item.employee?.department}</div>
                        <div className="text-xs text-gray-500">{item.employee?.role}</div>
                      </td>
                      <td className="px-6 py-4">
                         <div className="text-sm text-gray-800 font-medium">{item.taskName}</div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                          <div className="flex items-center space-x-3">
                             <div className="flex flex-col items-center min-w-[40px]">
                                <span className="text-sm font-bold text-gray-900">{item.overallScore}</span>
                             ...
                             <span className={`px-2 py-0.5 rounded text-xs font-bold w-8 text-center border ${
                              ...
                              item.grade === 'B' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-orange-50 text-orange-700 border-orange-200'
                            }`}>
                              {item.grade}
                            </span>
                          </div>
            ...
            {filteredRecords.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                    ...
                    <p className="text-sm">未找到符合筛选条件的考核记录</p>
                </div>
            )}
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center shrink-0">
           <div className="text-sm text-gray-600 font-medium flex items-center">
             已选择 <span className="text-blue-600 font-bold text-lg mx-1">{selectedRecordIds.size}</span> 个考核任务
           </div>
